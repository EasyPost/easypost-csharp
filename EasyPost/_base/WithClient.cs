...
namespace EasyPost._base
{
    public abstract class WithClient
    {
        ...
        /// <param name="overrideApiVersion">Override API version hit for HTTP request. Defaults to ApiVersion.General</param>
        /// <typeparam name="T">Type of object to return from request.</typeparam>
        /// <returns>A T-type object.</returns>
        protected async Task<T> Request<T>(Method method, string url, Dictionary<string, object>? parameters = null, string? rootElement = null, ApiVersion? overrideApiVersion = null) where T : class => await Client!.Request<T>(method, url, overrideApiVersion ?? ApiVersion.General, parameters, rootElement);
        ...
        protected async Task Request(Method method, string url, Dictionary<string, object>? parameters = null, ApiVersion? overrideApiVersion = null) => await Client!.Request(method, url, overrideApiVersion ?? ApiVersion.General, parameters);
