...
public class WebhookService : EasyPostService
{
    ...
    public async Task<Webhook> Update(string webhookId, Dictionary<string, object>? parameters = null) => await Request<Webhook>(Method.Patch, $"webhooks/{webhookId}", parameters);
    ...
    public async Task Delete(string webhookId) => await Request(Method.Delete, $"webhooks/{webhookId}");
