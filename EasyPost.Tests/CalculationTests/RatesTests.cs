...
namespace EasyPost.Tests.CalculationTests
{
    public class RatesTests
    {
        ...
        public void TestGetLowestRateObject()
        {
            ...
            Rate lowestRate = Calculation.Rates.GetLowestObjectRate(rates);
        ...
        public void TestLowestRateFilters()
        {
            ...
            lowestRate = Calculation.Rates.GetLowestObjectRate(rates, new List<string> { "CarrierB" });
            Assert.Equal("1.00", lowestRate.Price);
            ...
            lowestRate = Calculation.Rates.GetLowestObjectRate(rates, includeServices: new List<string> { "ServiceB" });
            Assert.Equal("1.00", lowestRate.Price);
            ...
            lowestRate = Calculation.Rates.GetLowestObjectRate(rates, new List<string> { "CarrierB" }, new List<string> { "ServiceB" });
            Assert.Equal("1.00", lowestRate.Price);
            Assert.Equal("CarrierB", lowestRate.Carrier);

            // It may go with a higher price if filters are involved
            lowestRate = Calculation.Rates.GetLowestObjectRate(rates, new List<string> { "CarrierC" });
            Assert.Equal("100.00", lowestRate.Price);
            Assert.Equal("CarrierC", lowestRate.Carrier);

            lowestRate = Calculation.Rates.GetLowestObjectRate(rates, includeServices: new List<string> { "ServiceC" });
            Assert.Equal("100.00", lowestRate.Price);
        ...
        public void TestLowestRateFilteringError()
        {
            ...
            Assert.Throws<FilteringError>(() => Calculation.Rates.GetLowestObjectRate(rates));
        }
